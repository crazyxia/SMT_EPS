 ...
 */
public class ConnectToCenterRemote {
	 ...
	 * Description: 进行初始化操作
	 */
	public ConnectToCenterRemote(int line, CenterLoginMapper centerLoginMapper, SocketLogMapper socketLogMapper, CenterStateMapper centerStateMapper) throws Exception {
		this.line = line;
	...
	public synchronized void sendCmdToConvery(ClientDevice clientDevice, boolean isAlarm) throws Exception {
		if (converyPaused ^ isAlarm) {
			...
			ControlReplyPackage controlReplyPackage = centerRemote.receiveControlPackage(controlPackage);
			// 创建日志：收到的包
