...
public class AlarmConnectToSocket {
     ...
     * 需要监听的端口
     */
    private int listen_alarm_port;
    
    public AlarmConnectToSocket() {
        IniReader.setIni(INI_CONFIG_PATH);
        Map<String, String> map = IniReader.getItem("port");
        listen_alarm_port = Integer.parseInt(map.get("listen_alarm_port"));
        communicator = new SyncCommunicator(listen_alarm_port, PACKAGE_PATH);
    }
